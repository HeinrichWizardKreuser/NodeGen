...
public final class CoreDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener, MouseWheelListener {
...
//some personal edits
    ...
    public void setTitle() {
      setTitle("Pane " + (amountOfPanes + 1));
    }
    public static String title = "Pane";
    private static int amountOfPanes = 0;
    public static int spawnY = 0, spawnX = 0;
    //customized colors
    ...
    private Color penColor;// current pen color
    ...
    private final double DEFAULT_PEN_RADIUS = 0.002;// default pen radius
    private double penRadius;// current pen radius
    private boolean defer = false;// show we draw immediately or wait until next show?
    ...
    // manually setup the canvas size and dimensions
    public CoreDraw() {
      int w = DEFAULT_SIZE, h = DEFAULT_SIZE;
      checkDimensions(w, h);
      int pixls = optimalPixls(w, h);
      setTitle();
      setup(w, h, pixls);
    }

    // given w and h, creates a canvas with pixls optimized to the screen resolution
    public CoreDraw(int w, int h) {
      checkDimensions(w, h);
      int pixls = optimalPixls(w, h);
      setTitle();
      setup(w, h, pixls);
    ...
    public CoreDraw(int w, int h, String title) {
      checkDimensions(w, h);
      int pixls = optimalPixls(w, h);
      setTitle(title);
      setup(w, h, pixls);
    ...
    public CoreDraw(int w, int h, int pixls) {
      checkDimensions(w, h);
      checkPixls(pixls);
      setTitle();
      setup(w, h, pixls);
    ...
    public CoreDraw(int w, int h, int pixls, String title) {
      checkPixls(pixls);
      checkDimensions(w, h);
      setTitle(title);
      setup(w, h, pixls);
    ...
    // given correct values for w, h and pixls, sets up the pane
    private void setup(int w, int h, int pixls) {
      width = w * pixls;
      height = h * pixls;
      frameW = w;
      frameH = h;
      ...
      draw.addMouseWheelListener(this);//---------------PERSONAL ADD
      ...
      // set location to curr
      frame.setLocation(spawnX, spawnY);
      // calc new spawn location
      spawnX += w * pixls;
    ...
    private void checkDimensions(int w, int h) {
      if (w <= 0 || h <= 0) {
        throw new IllegalArgumentException("width and height must be positive");
      }
    }

    private void checkPixls(int pixls) {
      if (pixls <= 0) {
        throw new IllegalArgumentException("pixels must be positive");
      }
    }

    private int optimalPixls(int w, int h) {
      // optimize the pixls for the screen size
      ...
      int pixls = (int) (Math.min(screenW / w, screenH / h) * 0.9);
      checkPixls(pixls);
      return pixls;
    }
    ...
    ***************************************************************************/
     ...
     * {@code CoreDraw.BLACK}, {@code CoreDraw.BLUE}, {@code CoreDraw.CYAN},
     * {@code CoreDraw.DARK_GRAY}, {@code CoreDraw.GRAY}, {@code CoreDraw.GREEN},
     * {@code CoreDraw.LIGHT_GRAY}, {@code CoreDraw.MAGENTA}, {@code CoreDraw.ORANGE},
     * {@code CoreDraw.PINK}, {@code CoreDraw.RED}, {@code CoreDraw.WHITE}, and
     * {@code CoreDraw.YELLOW}.
     *
    ...
    public void setPencolor(Color color) { setPenColor(color); }
    ...
    ***************************************************************************/
    ...
    public void line(double x0, double y0, double x1, double y1, Color color){
        setPencolor(color);
        line(x0, y0, x1, y1);
    }
    ...
    public void point(Point p, Color color){
        setPencolor(color);
        point(p.x, p.y);
    }
    ...
    public void circle(Point p, double radius) {
        circle(p.x, p.y, radius);
    }
    public void circle(Point p, double radius, Color color) {
        setPenColor(color);
        circle(p.x, p.y, radius);
    }

    public void circle(double x, double y, double radius, double penRadiusMultiplier) {
    ...
    public void filledCircle(Point p, double radius) {
    ...
    public void filledCircle(Point p, double radius, Color color) {
      setPenColor(color);
      filledCircle(p.x, p.y, radius);
    }
    ...
    public void square(double x, double y, double halfLength, Color color){
        setPencolor(color);
    ...
    public void filledSquare(double x, double y, double halfLength, Color color){
        setPencolor(color);
        filledSquare(x, y, halfLength);
    }
    public void filledSquare(Point p, double halfLength){
        filledSquare(p.x, p.y, halfLength);
    }
    public void filledSquare(Point p, double halfLength, Color color){
        setPencolor(color);
        filledSquare(p.x, p.y, halfLength);
    }
    ...
    public void rectangle(double x, double y, double halfWidth, double halfHeight, Color color){
        setPencolor(color);
    ...
    public void filledRectangle(double x, double y, double halfWidth, double halfHeight, Color color){
        setPencolor(color);
    ...
    public void polygon(double[] x, double[] y, Color newColor){
        setPencolor(newColor);
        polygon(x, y);
    }

    public void polygon(Point[] poly){
    ...
    public void polygon(Point[] poly, Color newColor){
        setPencolor(newColor);
        polygon(poly);
    }

    public void polygon(ArrayList<Point> poly, Color newColor){
        setPencolor(newColor);
    //convert to collection of xs and ys and pass as polygon to be drawn
    ...
    public void filledPolygon(double[] x, double[] y, Color newColor){
        setPencolor(newColor);
        filledPolygon(x, y);
    }

    public void filledPolygon(Point[] poly, Color newColor){
        setPencolor(newColor);
    //convert to collection of xs and ys and pass as polygon to be drawn
        double xs[] = new double[poly.length];
        double ys[] = new double[poly.length];
    ...
    public void filledPolygon(ArrayList<Point> poly, Color newColor){
        setPencolor(newColor);
    //convert to collection of xs and ys and pass as polygon to be drawn
        double xs[] = new double[poly.size()];
        double ys[] = new double[poly.size()];
    ...
    *  Drawing Graphs
    ***************************************************************************/
    public void drawGraph(HashMap<Point, ArrayList<Point>> graph, double radius) {
    ...
    public void drawGraph(HashMap<Point, ArrayList<Point>> graph, double radius, Color color) {
      setPenColor(color);
      for (Point p : graph.keySet()) {
        filledCircle(p, radius);
        for (Point adj : graph.get(p)) {
    ...
    public void drawEdges(HashMap<Point, ArrayList<Point>> graph) {
      for (Point p : graph.keySet()) {
        for (Point adj : graph.get(p)) {
          line(p, adj);
        }
    ...
    public void drawEdges(HashMap<Point, ArrayList<Point>> graph, Color color) {
      setPenColor(color);
      for (Point p : graph.keySet()) {
        for (Point adj : graph.get(p)) {
          line(p, adj);
    ...
    ***************************************************************************/
    ...
    private Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = CoreDraw.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
...
*/
    ...
    public void picture(double x, double y, Image image, double scaledWidth, double scaledHeight) {
        if (scaledWidth  < 0) throw new IllegalArgumentException("width  is negative: " + scaledWidth);
        if (scaledHeight < 0) throw new IllegalArgumentException("height is negative: " + scaledHeight);
        double xs = scaleX(x);
        double ys = scaleY(y);
    ...
    ***************************************************************************/
    ...
    public void text(Point p, String text) {
      text(p.x, p.y, text);
    }
    public void text(Point p, String text, Color color) {
      setPencolor(color);
      text(p.x, p.y, text);
    }
    ...
    public void textLeft(double x, double y, String text, Color color){
        setPencolor(color);
        textLeft(x, y, text);
    }
    ...
    public void textRight(double x, double y, String text, Color color){
        setPencolor(color);
        textRight(x, y, text);
    }
...
//Check if the mousewheel has been moved
    public boolean mouseWheelMoved() {
      return mouseScrolls != 0;
    }
...
//Return the amount of units by which the mouse has been scrolle
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = this.userX(e.getX());
            mouseY = this.userY(e.getY());
            isMousePressed = true;
        }
    }
